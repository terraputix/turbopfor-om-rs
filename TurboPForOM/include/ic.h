...
//     static ALWAYS_INLINE unsigned bitgetx32(const unsigned char *__restrict in, unsigned idx, unsigned b)
//     {
//         unsigned bidx = b * idx;
...
//     static ALWAYS_INLINE unsigned bitgetx8(const unsigned char *__restrict in, unsigned idx, unsigned b)
//     {
//         unsigned bidx = b * idx;
//         return bzhi32(ctou16((uint16_t *)in + (bidx >> 4)) >> (bidx & 0xf), b);
//     }
...
//     static ALWAYS_INLINE unsigned bitgetx16(const unsigned char *__restrict in, unsigned idx, unsigned b)
//     {
//         unsigned bidx = b * idx;
//         return bzhi32(ctou32((uint32_t *)in + (bidx >> 4)) >> (bidx & 0xf), b);
//     }
...
//     static ALWAYS_INLINE void bitsetx16(const unsigned char *__restrict in, unsigned idx, unsigned v, unsigned b)
//     {
//         unsigned bidx = b * idx;
//         unsigned *p = (unsigned *)in + (bidx >> 4);
//         *p = (*p & ~(((1u << b) - 1) << (bidx & 0xf))) | v << (bidx & 0xf);
//     }
//     static ALWAYS_INLINE void bitsetx32(const unsigned char *__restrict in, unsigned idx, unsigned v, unsigned b)
//     {
//         unsigned bidx = b * idx;
//         unsigned long long *p = (unsigned long long *)((unsigned *)in + (bidx >> 5));
//         *p = (*p & ~(((1ull << b) - 1) << (bidx & 0x1f))) | (unsigned long long)v << (bidx & 0x1f);
//     }
// #endif
...
// #define P4HVE(_out_, _b_, _bx_, _usize_)                          \
//     do                                                            \
//     {                                                             \
//         if (!_bx_)                                                \
//             *_out_++ = _b_;                                       \
//         else if (_bx_ <= _usize_)                                 \
//             *_out_++ = 0x80 | _b_, *_out_++ = _bx_;               \
//         else                                                      \
//             *_out_++ = (_bx_ == _usize_ + 1 ? 0x40 : 0xc0) | _b_; \
//     } while (0)
...
// #define P4HVE64(_out_, _b_, _bx_)               \
//     do                                          \
//     {                                           \
//         unsigned _c = _b_ == 64 ? 64 - 1 : _b_; \
//         P4HVE(_out_, _c, _bx_, 64);             \
//     } while (0)
...
//     static inline unsigned p4bits(unsigned char *__restrict in, int *bx)
//     {
//         unsigned i = ctou16(in);
//         *bx = P4D_XB(i);
//         return P4D_B(i);
//     }
...
//     static inline void p4ini(struct p4 *p4, unsigned char **pin, unsigned n, unsigned *b)
//     {
...
//             {
//                 p4->cum[j] = num;
//                 num += popcnt64(ctou64(in + j * 8));
//             }
//             if (n & 0x3f)
//                 num += popcnt64(ctou64(in + j * 8) & ((1ull << (n & 0x3f)) - 1));
//             p4->ex = p = in + (n + 7) / 8;
...
//     static ALWAYS_INLINE uint8_t p4getx8(struct p4 *p4, unsigned char *in, unsigned idx, unsigned b)
//     {
//         unsigned bi, cl, u = bitgetx8(in, idx, b);
//         if (p4->xmap[bi = idx >> 6] & (1ull << (cl = idx & 63)))
//             u += bitgetx8(p4->ex, p4->cum[bi] + popcnt64(p4->xmap[bi] & ~(~0ull << cl)), p4->bx) << b;
//         return u;
//     }

//     static ALWAYS_INLINE uint16_t p4getx16(struct p4 *p4, unsigned char *in, unsigned idx, unsigned b)
//     {
//         unsigned bi, cl, u = bitgetx16(in, idx, b);
//         if (p4->xmap[bi = idx >> 6] & (1ull << (cl = idx & 63)))
//             u += bitgetx16(p4->ex, p4->cum[bi] + popcnt64(p4->xmap[bi] & ~(~0ull << cl)), p4->bx) << b;
//         return u;
//     }
//     static ALWAYS_INLINE uint32_t p4getx32(struct p4 *p4, unsigned char *in, unsigned idx, unsigned b)
//     {
//         unsigned bi, cl, u = bitgetx32(in, idx, b);
//         if (p4->xmap[bi = idx >> 6] & (1ull << (cl = idx & 63)))
//             u += bitgetx32(p4->ex, p4->cum[bi] + popcnt64(p4->xmap[bi] & ~(~0ull << cl)), p4->bx) << b;
//         return u;
//     }

//     // Get the next single value greater of equal to val
//     static ALWAYS_INLINE uint16_t p4geqx8(struct p4 *p4, unsigned char *in, unsigned b, uint8_t val)
//     {
//         do
//             p4->oval += p4getx8(p4, in, ++p4->idx, b) + 1;
//         while (p4->oval < val);
//         return p4->oval;
//     }
//     static ALWAYS_INLINE uint16_t p4geqx16(struct p4 *p4, unsigned char *in, unsigned b, uint16_t val)
//     {
//         do
//             p4->oval += p4getx16(p4, in, ++p4->idx, b) + 1;
//         while (p4->oval < val);
//         return p4->oval;
//     }
//     static ALWAYS_INLINE uint32_t p4geqx32(struct p4 *p4, unsigned char *in, unsigned b, uint32_t val)
//     {
//         do
//             p4->oval += p4getx32(p4, in, ++p4->idx, b) + 1;
//         while (p4->oval < val);
//         return p4->oval;
//     }
