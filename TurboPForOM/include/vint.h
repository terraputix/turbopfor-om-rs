...
#define _vbxput32(_op_, _x_, _act_)                      \
  {                                                      \
    ...
    {                                                    \
      ctou16(_op_) = bswap16((_x_) | 0x8000u);           \
      _op_ += 2;                                         \
      _act_;                                             \
    }                                                    \
...
#define _vbxget32(_ip_, _x_, _act_)                                  \
  do                                                                 \
  {                                                                  \
    _x_ = (unsigned)(*_ip_++);                                       \
    if (!(_x_ & 0x80u))                                              \
...
#define _vbxget64(_ip_, _x_, _act_)                                                           \
  do                                                                                          \
  {                                                                                           \
    _x_ = *_ip_++;                                                                            \
    if (!(_x_ & 0x80))                                                                        \
...
#define vbxput64(_op_, _x_)      \
  {                              \
    unsigned long long _x = _x_; \
    _vbxput64(_op_, _x, ;);      \
  }
#define vbxput32(_op_, _x_)     \
  {                             \
...
#define _vblen32(_x_) ((_x_) < VB_OFS1 ? 1 : ((_x_) < VB_OFS2 ? 2 : ((_x_) < VB_OFS3) ? 3 \
                                                                                      : (bsr32(_x_) + 7) / 8 + 1))
#define _vbvlen32(_x_) ((_x_) < VB_OFS1 ? 1 : ((_x_) < VB_BA2 ? 2 : ((_x_) < VB_BA3) ? 3 \
                                                                                     : (_x_ - VB_BA3)))
...
#define _vbput32(_op_, _x_, _act_)                                                        \
  {                                                                                       \
    ...
    {                                                                                     \
      ctou16(_op_) = bswap16((VB_OFS1 << 8) + ((_x_) - VB_OFS1));                         \
      _op_ += 2; /*(_x_) -= VB_OFS1; *_op_++ = VB_OFS1 + ((_x_) >> 8); *_op_++ = (_x_);*/ \
      _act_;                                                                              \
    }                                                                                     \
    else if ((_x_) < VB_OFS3)                                                             \
    {                                                                                     \
      *_op_++ = VB_BA2 + (((_x_) -= VB_OFS2) >> 16);                                      \
      ctou16(_op_) = (_x_);                                                               \
      _op_ += 2;                                                                          \
      _act_;                                                                              \
    }                                                                                     \
...
#define _vbget32(_ip_, _x_, _act_)                                                           \
  do                                                                                         \
  {                                                                                          \
    _x_ = *_ip_++;                                                                           \
    if (likely(_x_ < VB_OFS1))                                                               \
    ...
    else if (likely(_x_ < VB_BA2))                                                           \
    {                                                                                        \
      _x_ = /*bswap16(ctou16(_ip_-1))*/ ((_x_ << 8) + (*_ip_)) + (VB_OFS1 - (VB_OFS1 << 8)); \
      _ip_++;                                                                                \
      _act_;                                                                                 \
    }                                                                                        \
    else if (likely(_x_ < VB_BA3))                                                           \
    {                                                                                        \
      _x_ = ctou16(_ip_) + ((_x_ - VB_BA2) << 16) + VB_OFS2;                                 \
      _ip_ += 2;                                                                             \
      _act_;                                                                                 \
    }                                                                                        \
    else                                                                                     \
    {                                                                                        \
      unsigned _b = _x_ - VB_BA3;                                                            \
...
#define _vbget64(_ip_, _x_, _act_)                                                           \
  do                                                                                         \
  {                                                                                          \
    _x_ = *_ip_++;                                                                           \
    if (likely(_x_ < VB_OFS1))                                                               \
    ...
    {                                                                                        \
      unsigned _b = _x_ - VB_BA3;                                                            \
      _x_ = ctou64(_ip_) & ((1ull << 8 * _b << 24) - 1);                                     \
      _ip_ += 3 + _b;                                                                        \
      _act_;                                                                                 \
    }                                                                                        \
  } while (0)
...
#define _leb128get(_ip_, _x_, _act_) \
  {                                  \
    unsigned _sft = 0;               \
    for (_x_ = 0;; _sft += 7)        \
    {                                \
...
#define _vbput8(_op_, _x_, _act_) \
  {                               \
    *_op_++ = _x_;                \
    _act_;                        \
  }
#define _vbget8(_ip_, _x_, _act_) \
  {                               \
    _x_ = *_ip_++;                \
    _act_;                        \
  }
...
#define vbput64(_op_, _x_)       \
  {                              \
    unsigned long long _x = _x_; \
    _vbput64(_op_, _x, ;);       \
  }
#define vbput32(_op_, _x_)      \
  {                             \
    register unsigned _x = _x_; \
    _vbput32(_op_, _x, ;);      \
  }
